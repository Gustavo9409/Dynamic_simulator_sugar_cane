...
class update_flow_data():
	...
	def update_vapor(self, Pvin):
		...
		pv = vapor_prpty.density(Pvin)

		uv = vapor_prpty.viscosity(Tvin)

		Hv = vapor_prpty.enthalpy(Tvin,Pvin)

		Yv = vapor_prpty.thermal_conductivity(Tvin)

		Cpv = Hv/Tvin

		Hvw = Hv - water_prpty.enthalpy(Tvin)
...
class juice():
	...
	def mass(self):
		...
		#Water component of juice [kg/s]
		self.water_f = self.Mj*(1 - self.Bj - self.Ij)
		#Insoluble solides component of juice [kg/s]
		self.insoluble_f = self.Mj*self.Ij
		#Sucrose component of juice [kg/s]
		self.sucrose_f = self.Mj*(self.Bj*self.Zj)
		#No sucrose component of juice [kg/s]
		self.nosucrose_f =self.Mj*(self.Bj*(1-self.Zj))
		#Solids component of juice [kg/s]
