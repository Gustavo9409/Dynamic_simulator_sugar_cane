...
from backend_bases_DynamicSim import FigureManagerBase
from backend_bases_DynamicSim import FigureCanvasBase
from backend_bases_DynamicSim import NavigationToolbar2

from backend_bases_DynamicSim import cursors
from backend_bases_DynamicSim import TimerBase
from backend_bases_DynamicSim import ShowBase
...
class NavigationToolbar2QT(NavigationToolbar2, QtWidgets.QToolBar):
	...
	def _init_toolbar(self):
		...
		for text, tooltip_text, image_file, callback,arguments in self.toolitems:
			if text is None:
				...
				if callback in ['zoom', 'pan','V_cursor']:
					a.setCheckable(True)
		...
		self.init_x,self.init_y=self.initial_conditions()
	if is_pyqt5():
	...
	def initial_conditions(self):
		...
		for axes in allaxes:
			par_x.append(axes.get_xlim())
			par_y.append(axes.get_ylim())
		return par_x,par_y



	def Home_edit(self):
		# inti_ax=initial_conditions
		mins_x=[]
		maxs_x=[]
		mins_y=[]
		maxs_y=[]
		for xmins_xmaxs in self.init_x:
			mins_x.append(xmins_xmaxs[0])
			maxs_x.append(xmins_xmaxs[1])
		for ymins_ymaxs in self.init_y:
			mins_y.append(ymins_ymaxs[0])
			maxs_y.append(ymins_ymaxs[1])

		allaxes = self.canvas.figure.get_axes()
		for axes in allaxes:
			for s,st in enumerate(mins_x):

				axes.set_xlim(mins_x[s],maxs_x[s])
			for s,dt in enumerate(mins_y):

				axes.set_ylim(mins_y[s],maxs_y[s])
		self.canvas.draw()
	...
	def V_cursor(self,*args):
		...
		self._update_buttons_checked_2()
	...
	def Edit_axis(self):
		...
		if len(allaxes) == 1:
			...
			for axes in allaxes:
				...
				titles.append(name)
			item, ok = QtWidgets.QInputDialog.getItem(
				self.parent, 'Modificar', 'Seleccione el eje:', titles, 0, False)
			if ok:
				axes = allaxes[titles.index(six.text_type(item))]
			else:
				return
